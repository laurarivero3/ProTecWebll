...
$sentencia = $conexion->prepare("SELECT id, NombreMateria FROM Materias"); 
$sentencia->execute(); 
...
if($_POST){
    ...
    $idmaterias=(isset($_POST["idmaterias"])?$_POST["idmaterias"]:""); 
    $fechaIngreso=(isset($_POST["fechaIngreso"])?$_POST["fechaIngreso"]:""); 
    //preparar la insercion de los datos 
    //$sentencia=$conexion->prepare(query:"INSERT INTO Materias(id, NombreMateria)
    //values (null, \:NombreMateria)");

    $sentencia = $conexion->prepare("INSERT INTO Docentes(id, primerNombre, segundoNombre, primerApellido, segundoApellido,
    foto, cv, idmaterias, fechaIngreso) 
    VALUES (null, :primerNombre, :segundoNombre, :primerApellido, :segundoApellido, :foto, :cv, :idmaterias, :fechaIngreso)");
    ...
    $sentencia->bindParam(param: ":idmaterias", var: $idmaterias);
    $sentencia->bindParam( param:":fechaIngreso", var: $fechaIngreso);


    $sentencia->execute();
    $mensaje="Docente agregado exitosamente";
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                <label for="cv" class="form-label">Programa De Asignatura (PDF) </label>
             ...
             <!--<div class="mb-3">
                <label for="idmaterias" class="form-label"> Puesto</label>
                <input type="text" class="form-control" name="idmaterias" id="idmaterias" aria-describedby="helpId" placeholder="Colocar el puesto ">
                <small id="helpId" class="form-text text-muted">Ingrese el Puesto</small>
            </div> -->
            <div class="mb-3">
                <label for="idmaterias" class="form-label"> Puesto </label>
               <select class="form-select form-select-sm" name="idmaterias" id="idmaterias">
                <option selected>Seleccione una opción</option>
                <?php foreach ($lista_Materias as $registro) { ?>
                    <option value="<?php echo $registro['NombreMateria']; ?>">
                        <?php echo $registro['NombreMateria']; ?>
                    </option>
                    <?php } ?>
                </select>
            </div>
